-- PetGestor BLOCO 6 hardening — validade civil da empresa no estoque/PDV.
-- Incremental. NÃO edita migrations anteriores (BLOCOs 1–6 inclusive).
-- Não reseta banco, não apaga vendas/estoque, não desabilita RLS.
-- Não inicia BLOCO 7 (relatórios).
--
-- Regra única (reutiliza private.company_civil_today do BLOCO 4):
--   expiration_date <  hoje civil da empresa → vencido / não vendável
--   expiration_date =  hoje civil da empresa → ainda válido naquele dia civil
--
-- CURRENT_DATE (data do servidor, tipicamente UTC) deixa de decidir lote vencido
-- em private.register_stock_movement e private.complete_product_sale.
-- due_date do financial_entry permanece CURRENT_DATE (não é regra de lote).
    ...
    FOR v_batch IN
      ...
      WHERE company_id = v_company_id AND product_id = v_product.id
        ...
        AND (v_exit_type = 'loss' OR v_exit_type = 'adjustment'
             OR expiration_date IS NULL OR expiration_date >= private.company_civil_today(v_company_id))
